...
try
  ...
  if length(fieldText)
    if strcmp(char(191), fieldText(1))
      ...
      if size(fieldText, 1) > 1
        fieldText(1,1:size(fieldText,2)-1) = fieldText(1,2:size(fieldText,2));
        fieldText(1,size(fieldText,2))= ' ';
      else % if size(fieldText, 1) > 1
        fieldText = fieldText(1,2:size(fieldText,2));
  ...
  if ~any(primaryNdx);
    ...
    primaryNdx = find( ismember({formField.PACFormTagPrimary}, lower(fieldID)) );
    if ~any(primaryNdx);
      % PacFORMS now include sender's date & name even if the Form doesn't have
      %   a field for these => not considered an error if not on the Form.
      if ~any(find( ismember({'opdate','opname','opcall'}, lower(fieldID)) ))
        err = 99;
        errMsg = sprintf('>%s: field "%s" not in list of formField.PACFormTagPrimary.', mfilename, fieldID);
        return
      end % if ~any(find( ismember({'opdate','opname'}, lower(fieldID)) ))
    end % if ~any(primaryNdx);
  end % if ~any(primaryNdx);
  % we are only here if (a) the fieldID exists for the form or 
  %                     (b) the field doesn't exist and that's OK/not an error.
  if any(primaryNdx)
      ...
      if length(char(formField(thisPage, primaryNdx).PACFormTagSecondary)) | b
        ...
        if (length(primaryNdx) > 1)
          ...
          if (length(h_field) > 1)
            ...
            else % if strcmp(formField(thisPage, primaryNdx).PACFormTagPrimary, 'msgno')
              [outstring,newpos] = textwrap(h_field(thisPage, primaryNdx), {fieldText});
              if expandBox
                positPrim = get(h_field(thisPage, primaryNdx(1)),'Position');
                expandBox = 2 * (newpos(4) > positPrim(4));
              end % if expandBox
            end % if strcmp(formField(thisPage, primaryNdx).PACFormTagPrimary, 'msgno') else
        ...
        if (expandBox > 1)
          ud = get( h_field(pageNdx, size(h_field, 2)),'UserData');
          if any(ismember(fieldnames(ud), 'formType'))
            if strcmp(char(ud.formType), 'simple')
              expandBox = 1;
              err = 100; %until code commented about on next line is in
              %open as many pages as needed to contain the simple message
            end
          end
        end
        if (expandBox > 1)

function [err, errMsg, badNewsText, detailBadNews] = checkCompilerMessages(fileName, displayFlag)
      ...
      if findstr(char(badNews(itemp)), lower_diaryText)
        ...
        if foundBadNews
          ...
          if badNewsCount <= badNewsCountLimit
            diary on
            detailBadNews(length(detailBadNews)+1) = badNewsText(badNewsCount);
            fprintf('\n') ;
            disp(diaryText);
            detailBadNews(length(detailBadNews)+1) = {fgetl(fid)};
            disp(detailBadNews(length(detailBadNews)) );
            diary off
          end
          break;
        end

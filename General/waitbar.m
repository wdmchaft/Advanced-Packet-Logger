...
%Last checkin   $Date: 3/08/05 10:20a $
%Last modify    $Modtime: 3/08/05 10:20a $
%Last changed by$Author: Arose $
%  $NoKeywords: $
%look for 'EDCB' for the changes/upgrades

%   Clay M. Thompson 11-9-92
%   Vlad Kolesnikov  06-7-99
%   Copyright 1984-2001 The MathWorks, Inc.
%   $Revision: 4 $  $Date: 3/17/03 3:24p $
...
switch type
 ...
 case 2,  % waitbar(x,name)  initialize
  ...
  f = figure(...
      ...
      'BusyAction', 'queue', ...
      'Position', pos, ...
      'Resize','on', ...%'Resize','off', ... EDCB patch for ML bug that amkes figure 0 height on fast CPUs
      'CreateFcn','', ...
      'NumberTitle','off', ...
      'IntegerHandle','off', ...
      'MenuBar', 'none', ...
  ...
  if nargin > 2,
      ...
      for ii = 1:length( propList )
          try
            ...
            if strcmp(char(propList{ii}), 'Interpreter') %EDCB
              titleProperty = 'Interpreter';
              titleValue = valueList{ii};
            else
  ...
  try%EDCB
    tHandle=title(name, titleProperty, titleValue);
  catch
    tHandle=title(name);
  end
  tHandle=get(h,'title');
  ...
  a = 0;
  if exist('propList')
    for itemp = 1:length(propList)
      if strcmp('visible',lower(char(propList(itemp))))
        a = 1;
        set(f,'HandleVisibility','callback',propList(itemp),valueList(itemp));
        break
      end
    end
  end

...
case 'logprt'
  ...
  tbLogPrtPane_Callback(handles.tbLogPrtPane, [], handles, []);
otherwise
...
% --------------------------------------------------------------------
function varargout = tbLogPrtPane_Callback(h, eventdata, handles, varargin)
val = get(h, 'val');
set(h, 'enable','inactive');
handles = alterPaneState(handles, 'off');
[err, errMsg, logPrtEnable, handles.logPrint_mntIntr, handles.logPrint_mnmToPrint, handles.logPrint_msgNums]...
  = readLogPrintINI(outpostValByName('DirAddOns', handles.outpostNmNValues));
set(handles.cbLogPrtEnable_0, 'value', logPrtEnable);
set(handles.editLogPrt_mntIntr, 'value', handles.logPrint_mntIntr);
set(handles.editLogPrt_mnmToPrt, 'value', handles.logPrint_mnmToPrint);
set(handles.editLogPrt_msgNums, 'value', handles.logPrint_msgNums);

cbLogPrtEnable_Callback(handles.cbLogPrtEnable_0, [], handles, [])
...
[err, errMsg] = writeLogPrintINI(outpostValByName('DirAddOns', handles.outpostNmNValues), ...
  logPrtEnable, handles.logPrint_mntIntr, handles.logPrint_mnmToPrint, handles.logPrint_msgNums);
% --------------------------------------------------------------------
...
% --------------------------------------------------------------------
function varargout = editLogPrt_mntIntr_Callback(h_obj, eventdata, handles, varargin)
[err] = getCheckNumValEditBox(h_obj, handles.logPrint_mntIntr, handles, 0);
if ~err
  setUpdated('LogPrt', handles)
end
% --------------------------------------------------------------------
function varargout = editLogPrt_mnmToPrt_Callback(h_obj, eventdata, handles, varargin)
[err] = getCheckNumValEditBox(h_obj, handles.logPrint_mnmToPrint, handles, 0);
if ~err
  setUpdated('LogPrt', handles)
end
% --------------------------------------------------------------------
function varargout = editLogPrt_msgNums_Callback(h_obj, eventdata, handles, varargin)
[err] = getCheckNumValEditBox(h_obj, handles.logPrint_mntIntr, handles, 1);
if ~err
  setUpdated('LogPrt', handles)
end
% --------------------------------------------------------------------
function varargout = cbLogPrtEnable_Callback(h, eventdata, handles, varargin)
val = get(h,'value');
if val
  enb = 'on';
  colr = [1 1 1];
...
set(handles.editLogPrt_mntIntr, 'enable', enb, 'BackgroundColor', colr);

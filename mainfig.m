...
% MAINFIG Application M-file for mainFig.fig
%    FIG = MAINFIG launch mainFig GUI.
%    MAINFIG('callback_name', ...) invoke the named callback.
...
if nargin < 2  % LAUNCH GUI
  if nargin < 1  % LAUNCH GUI
    fig = mainFig_OpeningFunction;
  else
    fig = mainFig_OpeningFunction(varargin{:});
  end
...
function varargout = mainFig_OpeningFunction(varargin)
...
% % set(handles.figure2,'CloseRequestFcn', 'mainFig(''figure1_CloseRequestFcn'',handles.figure1,[],guidata(handles.figure1))')
guidata(handles.figure1, handles);
%get the digitizer running: large figure, "bombsite" figure, initial load
digitizePoints(handles.figure1, handles.figure2, -1)

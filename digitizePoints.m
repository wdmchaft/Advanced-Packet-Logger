...
if ~keyMOUSEpress
  ...
  case 'n' %new group
    ...
    a = sprintf('%s\n  ex:\n   Enter 8:=FullAct for checkbox active when the response to field "8.:" is "[FullAct]"', a);
    a = sprintf('%s\n  ex:\n    where "[FullAct]" is the response generated by the PACF (you''ve got no control over PACF''s choice)', a);
    prompt = {sprintf('%s\n   Enter 8:=Limited for checkbox active when  the response to field "8.:" is " [Limited]"', a)};
    ...
    % started to put this in but realized the digitizer programs aren't set up
    %to use the handles structure so it will take more work to get the values returned!
%     if strcmp('Alignment', char(answer(1)));
%       handles.figure1 = figToTrack
%       figImageAlign(handles);
%     end
